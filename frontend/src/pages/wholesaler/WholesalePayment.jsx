import { useState } from 'react'
import { useLocation, useNavigate } from 'react-router-dom'
import { useAuth } from '../../context/AuthContext.jsx'
import { createInvoice } from '../../services/invoiceService'
import { clearCart } from '../../services/cartService.js'
import {
  CreditCard,
  Wallet,
  CalendarDays,
  IndianRupee,
  ShoppingBag,
  CheckCircle,
  Zap,
  ShieldCheck,
  Truck,
  Clock,
  HelpCircle
} from 'lucide-react'

export default function WholesalerPayment() {
  const navigate = useNavigate()
  const location = useLocation()
  const { user } = useAuth()

  const [paymentMethod, setPaymentMethod] = useState('cash')
  const [loading, setLoading] = useState(false)

  const totalAmount = location.state?.totalAmount || 0
  const cartItems = location.state?.cartItems || []
  const subTotal = location.state?.subTotal || 0
  const gstAmount = location.state?.gstAmount || 0

  const companyId = location.state?.company_id ?? cartItems[0]?.company_id ?? parseInt(user?.company_id || 0)
  const cashierId = location.state?.cashier_id ?? parseInt(user?.cashier_id ?? user?.id ?? 0)

  const paymentMethods = [
    {
      id: 'cash',
      title: 'Cash',
      icon: Wallet,
      description: 'Pay with physical cash',
      iconColor: 'text-green-600',
      bgLight: 'bg-green-50/60'
    },
    {
      id: 'upi',
      title: 'UPI',
      icon: Zap,
      description: 'Google Pay, PhonePe, etc.',
      iconColor: 'text-green-600',
      bgLight: 'bg-green-50/60'
    },
    {
      id: 'credit',
      title: 'Credit',
      icon: CalendarDays,
      description: 'Pay later on credit',
      iconColor: 'text-green-600',
      bgLight: 'bg-green-50/60'
    }
  ]

  const handlePayment = async () => {
    try {
      setLoading(true)

      const products = cartItems.map((item) => ({
        product_id: parseInt(item.product_id || item.id || 0),
        qty: parseInt(item.quantity || item.qty || 1),
      }))

      const payload = {
        company_id: companyId,

        customer_id: parseInt(user?.id || 0),
        wholesaler_id: parseInt(user?.id || 0),

        customer_name: user?.name || 'Guest',
        customer_phone: user?.phone || '9876543210',

        cashier_id: cashierId,

        products,

        sub_total: subTotal,
        gst_total: gstAmount,
        total_amount: totalAmount,

        payment_method: paymentMethod,
        payment_type: paymentMethod === 'credit' ? 'credit' : 'cash',

        gst_type: 'with_gst',

        paid_amount: paymentMethod === 'credit' ? 0 : totalAmount,
      }

      const response = await createInvoice(payload)

      if (response.status) {
        try {
          await clearCart(parseInt(user?.id || 21))
        } catch (clearError) {
          console.log('Cart clear error:', clearError)
        }

        window.dispatchEvent(new Event('cartUpdated'))

        const successDiv = document.createElement('div')
        successDiv.className = 'fixed top-4 right-4 z-50 animate-slide-in'
        successDiv.innerHTML = `
          <div class="flex items-center gap-3 bg-green-600 text-white px-6 py-4 rounded-xl shadow-lg border border-green-700">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
            </svg>
            <div>
              <p class="font-semibold text-sm">Payment Successful! ✅</p>
            </div>
          </div>
        `
        document.body.appendChild(successDiv)
        setTimeout(() => successDiv.remove(), 3000)

        navigate('/wholesaler/orders', { replace: true })
      } else {
        alert(response.message || 'Payment Failed')
      }
    } catch (error) {
      console.log(error)
      alert('Payment Failed ❌')
    } finally {
      setLoading(false)
    }
  }

  return (
    <div className="min-h-screen bg-white py-8 px-4 text-slate-800 font-sans">
      <div className="max-w-5xl mx-auto">

        {/* Header */}
        <div className="mb-10 text-left border-b border-green-100 pb-5">
          <div className="inline-block mb-2">
            <span className="bg-green-100 text-green-700 text-xs font-semibold px-3 py-1 rounded-full">
              Secure Checkout
            </span>
          </div>
          <div className="flex items-center gap-3 mt-2">
            <div className="p-2 bg-green-50 rounded-lg">
              <CreditCard className="w-6 h-6 text-green-600" />
            </div>
            <h1 className="text-2xl font-bold text-gray-800 tracking-tight">
              Complete Your Order
            </h1>
          </div>
          <p className="text-gray-500 text-sm pl-11 mt-1">Review your summary and select a payment method.</p>
        </div>

        {/* Balanced Grid System */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">

          {/* LEFT AREA - Steps & Options (7 Columns) */}
          <div className="lg:col-span-7 space-y-6">
            
            {/* Payment Options Segment */}
            <div className="bg-white rounded-xl border border-green-100 p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4 pb-3 border-b border-green-100">
                <Wallet className="w-5 h-5 text-green-600" />
                <h2 className="text-base font-semibold text-gray-800">Payment Method</h2>
              </div>

              <div className="space-y-2.5">
                {paymentMethods.map((method) => {
                  const Icon = method.icon
                  const isSelected = paymentMethod === method.id

                  return (
                    <div
                      key={method.id}
                      onClick={() => setPaymentMethod(method.id)}
                      className={`
                        flex items-center justify-between p-4 rounded-xl border cursor-pointer
                        transition-all duration-150 ease-in-out select-none
                        ${isSelected
                          ? `border-green-600 bg-green-50/30`
                          : 'border-green-100 hover:bg-green-50/20'
                        }
                      `}
                    >
                      <div className="flex items-center gap-3.5">
                        <div className={`p-2.5 rounded-lg ${isSelected ? 'bg-green-600 text-white' : 'bg-green-100 text-green-600'}`}>
                          <Icon className="w-5 h-5" />
                        </div>
                        <div>
                          <h4 className="font-semibold text-gray-800 text-sm">{method.title}</h4>
                          <p className="text-xs text-gray-500 mt-0.5">{method.description}</p>
                        </div>
                      </div>

                      {/* Radio Visual */}
                      <div className={`
                        w-5 h-5 rounded-full border flex items-center justify-center transition-colors
                        ${isSelected ? 'border-green-600 bg-white' : 'border-green-300 bg-white'}
                      `}>
                        {isSelected && (
                          <div className="w-2.5 h-2.5 rounded-full bg-green-600"></div>
                        )}
                      </div>
                    </div>
                  )
                })}
              </div>

              {/* Context Dynamic Alert Box */}
              <div className="mt-4 p-3.5 rounded-lg border border-green-100 bg-green-50/40">
                <div className="flex gap-2.5">
                  <ShieldCheck className="w-4 h-4 text-green-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-xs font-medium text-green-800">
                      {paymentMethod === 'cash' && 'Hand over the exact cash amount to the delivery executive.'}
                      {paymentMethod === 'upi' && 'Dynamic QR / Payment link will be verified immediately.'}
                      {paymentMethod === 'credit' && 'Post-paid account terms applied. Invoice will be issued.'}
                    </p>
                  </div>
                </div>
              </div>
            </div>

            {/* Customer Information Base */}
            <div className="bg-white rounded-xl border border-green-100 p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-3 pb-2 border-b border-green-100">
                <Truck className="w-5 h-5 text-green-600" />
                <h2 className="text-base font-semibold text-gray-800">Customer Profile</h2>
              </div>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-xs text-gray-400 block uppercase tracking-wider font-medium">Name</span>
                  <span className="font-medium text-gray-800">{user?.name || 'Guest'}</span>
                </div>
                <div>
                  <span className="text-xs text-gray-400 block uppercase tracking-wider font-medium">Phone</span>
                  <span className="font-medium text-gray-800">{user?.phone || '9876543210'}</span>
                </div>
              </div>
            </div>
          </div>

          {/* RIGHT AREA - Order Breakdown (5 Columns) */}
          <div className="lg:col-span-5">
            <div className="bg-white rounded-xl border border-green-100 overflow-hidden shadow-sm lg:sticky lg:top-6">
              
              {/* Card Title Box */}
              <div className="bg-green-50/30 border-b border-green-100 p-4 flex items-center justify-between">
                <div className="flex items-center gap-2 text-gray-800">
                  <ShoppingBag className="w-4 h-4 text-green-600" />
                  <h2 className="text-sm font-bold uppercase tracking-wider">Order Summary</h2>
                </div>
                <span className="text-xs bg-green-100 text-green-700 font-semibold px-2.5 py-0.5 rounded-full">
                  {cartItems.length} Items
                </span>
              </div>

              {/* Items Scroller Layout */}
              <div className="p-4 border-b border-green-100 max-h-48 overflow-y-auto divide-y divide-green-50">
                {cartItems.map((item, index) => (
                  <div key={index} className="flex justify-between items-center py-2.5 first:pt-0 last:pb-0">
                    <div className="pr-3">
                      <p className="font-medium text-gray-800 text-sm line-clamp-1">{item.product_name}</p>
                      <p className="text-xs text-gray-400 mt-0.5">Quantity: {item.quantity}</p>
                    </div>
                    <span className="font-semibold text-gray-800 text-sm flex-shrink-0">
                      ₹{(Number(item.price || 0) * Number(item.quantity || 0)).toFixed(2)}
                    </span>
                  </div>
                ))}
              </div>

              {/* Statement Pricing Panel */}
              <div className="p-4 bg-green-50/20 space-y-2.5 text-sm border-b border-green-100">
                <div className="flex justify-between text-gray-500">
                  <span>Subtotal</span>
                  <span className="text-gray-700 font-medium">₹{Number(subTotal).toFixed(2)}</span>
                </div>

                <div className="flex justify-between text-gray-500">
                  <span>GST (18%)</span>
                  <span className="text-gray-700 font-medium">₹{Number(gstAmount).toFixed(2)}</span>
                </div>

                <div className="flex justify-between text-gray-500">
                  <span>Delivery fee</span>
                  <span className="text-green-600 font-medium">Free</span>
                </div>

                <div className="flex justify-between pt-3 border-t border-green-100 items-baseline">
                  <span className="font-bold text-gray-800">Grand Total</span>
                  <div className="text-xl font-black text-green-600 flex items-center">
                    <IndianRupee className="w-4 h-4" />
                    <span>{Number(totalAmount).toFixed(2)}</span>
                  </div>
                </div>
              </div>

              {/* Footer CTA & Information Zone */}
              <div className="p-4 bg-white">
                <div className="flex items-center gap-2.5 text-gray-500 text-xs mb-4">
                  <Clock className="w-3.5 h-3.5 text-green-600" />
                  <span>Dispatching to logistics within 24 hours.</span>
                </div>

                {/* Pure Green Solid Button */}
                <button
                  onClick={handlePayment}
                  disabled={loading}
                  className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white py-3 rounded-lg text-sm font-bold shadow-sm transition-all duration-150 flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {loading ? (
                    <>
                      <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent"></div>
                      <span>Processing...</span>
                    </>
                  ) : (
                    <>
                      <CheckCircle className="w-4 h-4" />
                      <span>Place Order (₹{Number(totalAmount).toFixed(2)})</span>
                    </>
                  )}
                </button>

                <div className="text-center text-[11px] text-gray-400 mt-3.5 flex items-center justify-center gap-1">
                  <HelpCircle className="w-3 h-3" />
                  Secure checkout infrastructure
                </div>
              </div>

            </div>
          </div>

        </div>
      </div>

      <style>{`
        @keyframes slideIn {
          from { transform: translateX(100%); opacity: 0; }
          to { transform: translateX(0); opacity: 1; }
        }
        .animate-slide-in { animation: slideIn 0.2s ease-out; }
      `}</style>
    </div>
  )
}