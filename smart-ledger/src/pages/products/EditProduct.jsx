...
export default function EditProduct() {
  ...
  const handleImageChange = (e) => {
    ...
    setImageFile(file);
    setImagePreview(file ? URL.createObjectURL(file) : form.image || "");
    set("image", file ? file.name : form.image || "");
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await api.post("/product/update.php", body);
      if (res.data.status) {
  ...
  return (
    <>
      <style>{`
        ...
        .ep-file-input::-webkit-file-upload-button {
          border: none;
          background: #eef2ff;
          color: #1e293b;
          font: inherit;
              ...
              {fetching
                ...
                : <div className="ep-input-wrap">
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      className="ep-input ep-file-input"
                      onChange={handleImageChange}
                    />
                  </div>
              }
              {imagePreview && !fetching && (
